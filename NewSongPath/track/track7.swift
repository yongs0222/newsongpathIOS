...
class track7: UIViewController, FSCalendarDataSource, FSCalendarDelegate, FSCalendarDelegateAppearance{
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        let test = self.dateFormatter.date(from: "2020/07/01")
                cal.setCurrentPage(test!, animated: true)
        
           
           //Retrieving Data from Firebase
        Database.database().reference().child("track").child(uuid!).child("2020/07").observeSingleEvent(of:.value, with:{(snapshot) in
...
//                   let dataintemp = current[key] as! Bool
                   if value == false{
                       ...
                       self.cal.select(self.dateFormatter.date(from: "2020/07/\(key)"))
                   }
               }
           })
    }
